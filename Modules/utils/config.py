"""
Configuration utilities for Lexcode
------------------------------------
...
class Config:
    """Configuration manager for Lexcode."""
    ...
    def _load_from_env(self):
        """Load configuration from environment variables."""
        # API keys
        self.config_data['ai_api_key'] = os.getenv('LEXCODE_AI_API_KEY', '')
        self.config_data['vulners_api_key'] = os.getenv('LEXCODE_VULNERS_API_KEY', '')
        
        # Directories
        reports_dir = os.getenv('LEXCODE_REPORTS_DIR')
        if reports_dir:
            self.config_data['reports_dir'] = Path(reports_dir)
        else:
            self.config_data['reports_dir'] = self.project_root / 'reports'
        
        scans_dir = os.getenv('LEXCODE_SCANS_DIR')
        if scans_dir:
            self.config_data['scans_dir'] = Path(scans_dir)
        else:
            self.config_data['scans_dir'] = self.project_root / 'scans'
        
        templates_dir = os.getenv('LEXCODE_TEMPLATES_DIR')
        if templates_dir:
            self.config_data['templates_dir'] = Path(templates_dir)
        else:
            self.config_data['templates_dir'] = self.project_root / 'lexcode' / 'templates'
