"""
Advanced AI Report Generator using LangChain for Lexcode
--------------------------------------------------------
...
logger = logging.getLogger("lexcode.langchain_reporter")
...
class VulnerabilityReportGenerator:
    ...
    def _initialize_llm(self) -> Any:
        ...
        if os.getenv("LEXCODE_OPENAI_API_KEY"):
            return ChatOpenAI(
                openai_api_key=os.getenv("LEXCODE_OPENAI_API_KEY"),
                model_name="gpt-4",
                temperature=0.2
            )
            
        # Check for Anthropic API key
        elif os.getenv("LEXCODE_ANTHROPIC_API_KEY"):
            try:
                from langchain.llms import Anthropic
                return Anthropic(
                    anthropic_api_key=os.getenv("LEXCODE_ANTHROPIC_API_KEY"),
                    model="claude-2",
                    temperature=0.2
                )
            except ImportError:
        ...
        elif os.getenv("LEXCODE_OLLAMA_API_URL"):
            try:
                from langchain.llms import Ollama
                return Ollama(
                    base_url=os.getenv("LEXCODE_OLLAMA_API_URL"),
                    model="llama2",
                    temperature=0.2
                )
            except ImportError:
    ...
    def _extract_vulnerability_summary(self, json_path: str) -> Dict:
        ...
        from lexcode.utils.ai_reporter import extract_vulnerability_data
...
**Scanner:** {metadata.get('scanner', 'Lexcode')}
...
*This report was automatically generated by Lexcode using AI analysis.*
"""
        return markdown
    ...
    def _create_html_report(self, markdown_content: str) -> str:
        ...
        try:
            ...
            <body>
                {html_content}
                <footer>
                    <p><em>Generated by Lexcode on {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}</em></p>
                </footer>
            </body>
            </html>
            """
