#!/usr/bin/env python3
"""
Lexcode - Automated Security Scanning Tool
-------------------------------------------
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Lexcode - Automated Security Scanning Tool",
...
def show_version():
    ...
    print("https://github.com/yourusername/lexcode")
    sys.exit(0)
...
def generate_ai_report(json_path, target, scan_type):
    ...
    try:
        from lexcode.utils.simple_ai_reporter import generate_report as simple_report
        ...
        if ai_report_path:
            ...
            auto_ai = os.getenv("LEXCODE_AUTO_AI_REPORT", "false").lower() in ["true", "1", "yes", "y"]
            use_langchain = os.getenv("LEXCODE_USE_LANGCHAIN", "false").lower() in ["true", "1", "yes", "y"]
    ...
    try:
        use_langchain = os.getenv("LEXCODE_USE_LANGCHAIN", "false").lower() in ["true", "1", "yes", "y"]
        if use_langchain:
            from lexcode.utils.langchain_reporter import generate_report as langchain_report
    ...
    try:
        from lexcode.utils.ai_reporter import generate_report as original_report
...
def start_cli_mode(args):
    """Start Lexcode in CLI mode."""
    from lexcode.core.scanner import Scanner
    from lexcode.core.parser import TemplateParser
    ...
    if not args.target:
        ...
        print("Usage: lexcode <target> [options]")
        print("       lexcode --gui (for graphical interface)")
        sys.exit(1)
    ...
    if result['success']:
        ...
        if result.get('report_path') and os.path.exists(result['report_path']):
            ...
            auto_ai_report = os.getenv("LEXCODE_AUTO_AI_REPORT", "false").lower() in ["true", "1", "yes", "y"]
...
def start_gui_mode():
    """Start Lexcode in GUI mode with web server."""
    try:
        import uvicorn
        ...
        print("\n[+] Starting Lexcode web interface...")
